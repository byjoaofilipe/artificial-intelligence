...
class ParetoFrontGenerator:
    ...
    def __init__(self, datasets, num_runs=10, output_dir="output_pareto"):
        """
    ...
    def run_all(self, 
        ...
        """
        print("\n" + "=" * 80)
        print("PARETO FRONT GENERATOR - MULTIPLE RUNS")
        print("=" * 80)
        ...
        print(f"Lambda weights: λ1 from 1.0 to 0.0 (step -0.1), λ2 from 0.0 to 1.0 (step +0.1)")
        print("=" * 80)
        ...
        for dataset_idx, dataset_path in enumerate(self.datasets):
            ...
            try:
                ...
                # Run algorithms multiple times with different lambda combinations
    ...
    def _generate_pareto_plots(self):
        ...
        # Create single plot with all algorithms
        fig, ax = plt.subplots(figsize=(12, 8))
        
        # Define colors and styles for each algorithm
        algo_styles = {
            'MILP-CBC': {'color': '#d62728', 'marker': 'o', 'linewidth': 3, 'markersize': 10, 'alpha': 0.9, 'label': 'MILP (Optimal)'},
            'MH-ILS': {'color': '#1f77b4', 'marker': 's', 'linewidth': 1, 'markersize': 6, 'alpha': 0.6, 'label': 'ILS'},
            'MH-VNS': {'color': '#ff7f0e', 'marker': '^', 'linewidth': 1, 'markersize': 6, 'alpha': 0.6, 'label': 'VNS'},
            'MH-NoOpt-ILS': {'color': '#2ca02c', 'marker': 'D', 'linewidth': 1, 'markersize': 6, 'alpha': 0.6, 'label': 'ILS (No-Opt)'},
            'MH-NoOpt-VNS': {'color': '#9467bd', 'marker': 'v', 'linewidth': 1, 'markersize': 6, 'alpha': 0.6, 'label': 'VNS (No-Opt)'}
        }
        ...
        for algo_name, style in algo_styles.items():
            ...
            # Plot all points (lighter)
            ax.scatter(df_algo['fo1'], df_algo['fo2'], 
                      color=style['color'], 
                      marker=style['marker'],
                      s=style['markersize']**2 * 3,
                      alpha=style['alpha'] * 0.3,
                      edgecolors='none')
            
            # Plot Pareto front points and line
        ...
        ax.set_ylabel('fo2 (Workload Balance: Max Normalized Workload)', 
                     fontsize=13, fontweight='bold')
        ax.set_title('Pareto Front: Multi-Objective Patient Allocation\n(Lower is Better for Both Objectives)', 
                    fontsize=15, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3, linestyle='--')
        ax.legend(loc='best', fontsize=11, framealpha=0.9)
        
        # Add note about Pareto optimality
        note_text = "Bold lines: Pareto front (non-dominated solutions)\nFaded points: All solutions explored"
        ...
        # Save plot
        filename = f'{self.output_dir}/pareto_front_all_algorithms.png'
...
Examples:
  ...
  python pareto_front_generator.py -d data1.dat data2.dat --runs 10
  
  # Run on all .dat files in a directory
  python pareto_front_generator.py -d data/*.dat --runs 5
  
  # Custom time limits
  python pareto_front_generator.py -d data/*.dat --runs 10 --milp-time 600
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of runs per algorithm per dataset (different lambda weights). Default: 10"
    )
